...
@section('css')
    <link rel="stylesheet" href="{{ config('streams.assets_path') }}/css/database.css">
@stop

@section('page_header')
    <h1 class="page-title">
        <i class="streams-data"></i> Database
        <a href="{{ route('streams.database.create') }}" class="btn btn-success"><i class="streams-plus"></i>
            Create New Table</a>
    </h1>
@stop
    ...
    <div class="page-content container-fluid">
        <div class="row">
            <div class="col-md-12">
                ...
                <?php $dataTypes = RAD\Streams\Models\DataType::all(); ?>
                <?php $dataTypeNames = []; ?>
                ...
                <?php $arr = RAD\Streams\Facades\DBSchema::tables(); ?>
                @foreach($arr as $a)
	            <?php $table = current($a); ?>
		    <?php if (in_array($table, config('streams.database.tables.hidden', []))) continue; ?>
                	<?php $active = in_array($table, $dataTypeNames); 
                        if($active){
                            $activeDataType = RAD\Streams\Models\DataType::where('name', '=', $table)->first();
                        }
                    ?>
                        ...
                        <tr>
                            <td>
                                <p class="name">
                                    @if($active)
                                        <a href="{{ route('streams.database.show', $table) }}"
                            ...
                            <td class="actions">
                                <a class="btn-danger btn-sm pull-right delete_table @if($active) remove-bread-warning @endif"
                                   data-table="{{ $table }}" style="display:inline; cursor:pointer;"><i
                                            class="streams-trash"></i> Delete</a>
                                <a class="btn-sm btn-primary pull-right" style="display:inline; margin-right:10px;"
                                   href="{{ route('streams.database.edit', $table) }}"><i
                                            class="streams-edit"></i> Edit</a>
                                <a class="btn-sm btn-warning pull-right desctable"
                                   style="display:inline; margin-right:10px;"
                                   href="{{ route('streams.database.show', $table) }}" data-name="{{ $table }}"><i
                                            class="streams-eye"></i> View</a>
                            </td>
                        </tr>
                    @endforeach
                </table>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title"><i class="streams-trash"></i> Are you sure you want to delete the BREAD for
                        the <span id="delete_builder_name"></span> table?</h4>
                </div>
                <div class="modal-footer">
                    <form action="{{ route('streams.database.delete_bread', ['id' => null]) }}" id="delete_builder_form" method="POST">
                        {{ method_field('DELETE') }}
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                                aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title"><i class="streams-trash"></i> Are you sure you want to delete the <span
                                id="delete_table_name"></span> table?</h4>
                </div>
                <div class="modal-footer">
                    <form action="{{ route('streams.database.destroy', ['database' => '__database']) }}" id="delete_table_form" method="POST">
                        {{ method_field('DELETE') }}
                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                                aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title"><i class="streams-data"></i> @{{ table.name }}</h4>
                </div>
