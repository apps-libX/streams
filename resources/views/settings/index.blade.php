...
@section('css')
    <style>
        .panel-actions .streams-trash {
            cursor: pointer;
        }

        .panel-actions .streams-trash:hover {
            color: #e94542;
        }
...
@section('head')
    <script type="text/javascript" src="{{ config('streams.assets_path') }}/lib/js/jsonarea/jsonarea.min.js"></script>
@stop

@section('page_header')
    <h1 class="page-title">
        <i class="streams-settings"></i> Settings
    </h1>
@stop
...
@section('content')
    ...
    <div class="container-fluid">
        <div class="alert alert-info">
            ...
            <p>You can get the value of each setting anywhere on your site by calling <code>Streams::setting('key')</code></p>
        </div>
    </div>

    <div class="page-content container-fluid">
        <form action="{{ route('streams.settings.update') }}" method="POST" enctype="multipart/form-data">
            {{ method_field("PUT") }}
            {{ csrf_field() }}
            <div class="panel">
                @foreach($settings as $setting)
                    <div class="panel-heading">
                        <h3 class="panel-title">
                            {{ $setting->display_name }}<code>Streams::setting('{{ $setting->key }}')</code>
                        </h3>
                        <div class="panel-actions">
                            <a href="{{ route('streams.settings.move_up', $setting->id) }}">
                                <i class="sort-icons streams-sort-asc"></i>
                            </a>
                            <a href="{{ route('streams.settings.move_down', $setting->id) }}">
                                <i class="sort-icons streams-sort-desc"></i>
                            </a>
                            <i class="streams-trash"
                               data-id="{{ $setting->id }}"
                    ...
                    <div class="panel-body">
                        ...
                        @elseif($setting->type == "image" || $setting->type == "file")
                            @if(isset( $setting->value ) && !empty( $setting->value ) && Storage::exists(config('streams.storage.subfolder') . $setting->value))
                                <div class="img_settings_container">
                                    <a href="{{ route('streams.settings.delete_value', $setting->id) }}" class="streams-x"></a>
                                    <img src="{{ Storage::url(config('streams.storage.subfolder') . $setting->value) }}" style="width:200px; height:auto; padding:2px; border:1px solid #ddd; margin-bottom:10px;">
                                </div>
        ...
        <div class="panel" style="margin-top:10px;">
            <div class="panel-heading new-setting">
                <hr>
                <h3 class="panel-title"><i class="streams-plus"></i> New Setting</h3>
            </div>
            <div class="panel-body">
                <form action="{{ route('streams.settings.store') }}" method="POST">
                    {{ csrf_field() }}
                    <div class="col-md-4">
                    ...
                    <div class="col-md-12">
                        <a id="toggle_options"><i class="streams-double-down"></i> OPTIONS</a>
                    ...
                    <script>
                        $('document').ready(function () {
                            $('#toggle_options').click(function () {
                                ...
                                if ($('#toggle_options .streams-double-down').length) {
                                    $('#toggle_options .streams-double-down').removeClass('streams-double-down').addClass('streams-double-up');
                                } else {
                                    $('#toggle_options .streams-double-up').removeClass('streams-double-up').addClass('streams-double-down');
                                }
                            });
                        });
                    </script>
                    ...
                    <button type="submit" class="btn btn-primary pull-right new-setting-btn">
                        <i class="streams-plus"></i> Add New Setting
                    </button>
                    <div style="clear:both"></div>
                </form>
            </div>
            ...
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title">
                        <i class="streams-trash"></i> Are you sure you want to delete the <span id="delete_setting_title"></span> Setting?
                    </h4>
                </div>
                <div class="modal-footer">
                    <form action="{{ route('streams.settings.delete', ['id' => '__id']) }}" id="delete_form" method="POST">
                        {{ method_field("DELETE") }}
                        {{ csrf_field() }}
...
@section('javascript')
    ...
    <form id="my_form" action="{{ route('streams.upload') }}" target="form_target" method="POST" enctype="multipart/form-data" style="width:0;height:0;overflow:hidden">
        {{ csrf_field() }}
