        ...
        <div class="row">
            <div class="col-md-12">
                ...
                <div class="panel panel-bordered">
                    ...
                    <!-- form start -->
                    <form role="form"
                          action="@if(isset($dataTypeContent->id)){{ route('streams.'.$dataType->slug.'.update', $dataTypeContent->id) }}@else{{ route('streams.'.$dataType->slug.'.store') }}@endif"
                                ...
                                <div class="form-group">
                                    ...
                                    @elseif($row->type == "image" || $row->type == "file")
                                        @if($row->type == "image" && isset($dataTypeContent->{$row->field}))
                                            <img src="{{ Streams::image( $dataTypeContent->{$row->field} ) }}"
                                    ...
                                    @elseif($row->type == "select_dropdown")
                                        ...
                                        @if(isset($options->relationship))
                                            ...
                                            @if( !method_exists( $dataType->model_name, $row->field ) )
                                                <p class="label label-warning"><i class="streams-warning"></i> Make sure to setup the appropriate relationship in the {{ $row->field . '()' }} method of the {{ $dataType->model_name }} class.</p>
                                            @endif
                    ...
                    <form id="my_form" action="{{ route('streams.upload') }}" target="form_target" method="post"
