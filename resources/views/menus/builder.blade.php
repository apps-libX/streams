...
@section('css')
    <link rel="stylesheet" type="text/css" href="{{ config('streams.assets_path') }}/css/nestable.css">
@stop

@section('page_header')
    <h1 class="page-title">
        <i class="streams-list"></i>Menu Builder ({{ $menu->name }})
        <div class="btn btn-success add_item"><i class="streams-plus"></i> New Menu Item</div>
    </h1>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title"><i class="streams-trash"></i> Are you sure you want to delete this menu
                        item?</h4>
                </div>
                <div class="modal-footer">
                    <form action="{{ route('streams.menus.item.destroy', ['menu' => $menu->id, 'id' => '__id']) }}" id="delete_form"
                          method="POST">
                        {{ method_field("DELETE") }}
                        {{ csrf_field() }}
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                                aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title"><i class="streams-plus"></i> Create a New Menu Item</h4>
                </div>
                <form action="{{ route('streams.menus.item.add', ['menu' => $menu->id]) }}" id="delete_form" method="POST">
                    <div class="modal-body">
                        ...
                        <label for="icon_class">Font Icon class for the Menu Item (Use a <a
                                    href="{{ config('streams.assets_path') . '/fonts/streams/icons-reference.html' }}"
                                    target="_blank">Streams Font Class</a>)</label>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                                aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title"><i class="streams-edit"></i> Edit Menu Item</h4>
                </div>
                <form action="{{ route('streams.menus.item.update', ['menu' => $menu->id]) }}" id="edit_form" method="POST">
                    {{ method_field("PUT") }}
                    {{ csrf_field() }}
                    <div class="modal-body">
                        <label for="name">Title of the Menu Item</label>
...
@section('javascript')
    ...
    <script type="text/javascript" src="{{ config('streams.assets_path') }}/js/jquery.nestable.js"></script>
    <script>
        $(document).ready(function () {
            ...
            $('.dd').on('change', function (e) {
                $.post('{{ route('streams.menus.order',['menu' => $menu->id]) }}', {
