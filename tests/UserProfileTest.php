...
class UserProfileTest extends TestCase
{
    ...
    public function setUp()
    {
        ...
        $this->editPageForTheCurrentUser = route('streams.users.edit', ['user' => $this->user->id]);

        $this->listOfUsers = route('streams.users.index');
    }

    public function testCanSeeTheUserInfoOnHisProfilePage()
    {
        $this->visit(route('streams.profile'))
    ...
    public function testCanEditUserPassword()
    {
        $this->visit(route('streams.profile'))
             ...
             ->see('Edit User')
             ->seePageIs($this->editPageForTheCurrentUser)
             ->type('streams-rocks', 'password')
             ->press('Submit')
             ->seePageIs($this->listOfUsers);

        $updatedPassword = DB::table('users')->where('id', 1)->first()->password;
        $this->assertTrue(Hash::check('streams-rocks', $updatedPassword));
    }
