...
class StreamsBreadController extends Controller
{
    //***************************************
    //               ____
    //              |  _ \
    ...
    public function index(Request $request)
    {
        ...
        // Check permission
        Streams::can('browse_'.$dataType->name);
        ...
        $view = 'streams::bread.browse';

        if (view()->exists("streams::$slug.browse")) {
            $view = "streams::$slug.browse";
        }
    ...
    public function show(Request $request, $id)
    {
        ...
        // Check permission
        Streams::can('read_'.$dataType->name);

        $dataTypeContent = (strlen($dataType->model_name) != 0)
            ? call_user_func([$dataType->model_name, 'findOrFail'], $id)
            : DB::table($dataType->name)->where('id', $id)->first(); // If Model doest exist, get data from table name

        $view = 'streams::bread.read';

        if (view()->exists("streams::$slug.read")) {
            $view = "streams::$slug.read";
        }
    ...
    public function edit(Request $request, $id)
    {
        ...
        // Check permission
        Streams::can('edit_'.$dataType->name);

        $dataTypeContent = (strlen($dataType->model_name) != 0)
            ? call_user_func([$dataType->model_name, 'findOrFail'], $id)
            : DB::table($dataType->name)->where('id', $id)->first(); // If Model doest exist, get data from table name

        $view = 'streams::bread.edit-add';

        if (view()->exists("streams::$slug.edit-add")) {
            $view = "streams::$slug.edit-add";
        }
    ...
    public function create(Request $request)
    {
        ...
        // Check permission
        Streams::can('add_'.$dataType->name);
    ...
    public function destroy(Request $request, $id)
    {
        ...
        // Check permission
        Streams::can('delete_'.$dataType->name);
        ...
        return redirect()->route("streams.{$dataType->slug}.index")->with($data);
    }
}
