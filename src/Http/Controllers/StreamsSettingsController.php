...
class StreamsSettingsController extends Controller
{
    public function index()
    {
        // Check permission
        Streams::can('browse_settings');

        $settings = Setting::orderBy('order', 'ASC')->get();

        return view('streams::settings.index', compact('settings'));
    }
    ...
    public function delete_value($id)
    {
        ...
        if (isset($setting->id)) {
            ...
            if ($setting->type == 'image') {
                if (Storage::exists(config('streams.storage.subfolder').$setting->value)) {
                    Storage::delete(config('streams.storage.subfolder').$setting->value);
                }
            }
            $setting->value = '';
            $setting->save();
